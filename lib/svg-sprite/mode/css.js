...
SVGSpriteCss.prototype._addShapeToSimpleCssSprite = function(shape, needsRegular, index, position) {
	var dimensions					= shape.getDimensions(),
		...
		positionX					= 0,
		positionY					= 0,
		margin = shape.config.spacing.margin || 0;
	...
	switch (this.config.layout) {
		...
		case this.LAYOUT_HORIZONTAL:
			...
			this.data.spriteWidth	= Math.ceil(this.data.spriteWidth + dimensions.width) + margin;
		...
		case this.LAYOUT_DIAGONAL:
			...
			this.data.spriteWidth	= Math.ceil(this.data.spriteWidth + dimensions.width) + margin;
			this.data.spriteHeight	= Math.ceil(this.data.spriteHeight + dimensions.height) + margin;
			break;
